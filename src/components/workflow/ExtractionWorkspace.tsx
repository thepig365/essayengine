...
const EXTRACTION_TABS: ReadonlyArray<{ id: SourceMaterialPipelineTab; label: string }> = [
  ...
  { id: "link", label: "Link import" },
  { id: "paste", label: "Paste long text" },
  { id: "audio", label: "Upload audio" },
  { id: "document", label: "Text / transcript file" },
];
  ...
  return (
    ...
    >
      <div className="layer-head">
        ...
        <p>Every source becomes selectable text blocks before analysis or saving. Full source is not used by default.</p>
      </div>
      ...
      <label className="organize-option" style={{ marginTop: "0.75rem" }}>
        ...
        <span>Use full source (only when explicitly selected)</span>
      </label>
      <p className="transcript-note">
        When unchecked, analysis, custom extraction, and saved topic actions only use selected sections or paragraphs. To summarize a full video or page, select this option first or use “Replace Source with full source” below.
      </p>

      {sourceMaterialPipeline === "link" && (
        <div className="topic-filter" style={{ marginTop: "1rem" }}>
          <div className="range-head">
            <strong>Link import (LinkedIn / social / forums / podcast pages)</strong>
            <p>Extract readable page text and split it into paragraphs. Use the Transcript / captions tab for YouTube.</p>
          </div>
      ...
      {sourceMaterialPipeline === "paste" && (
        <div className="topic-filter" style={{ marginTop: "1rem" }}>
          <div className="range-head">
            ...
            <p>Split text into paragraph blocks, then select the parts you need.</p>
          </div>
          <label className="field">
            <span>Long text</span>
            <textarea
              ...
              placeholder="Paste an article, notes, thread text, or other long-form material…"
            />
          </label>
          <div className="range-actions cta-row">
      ...
      {sourceMaterialPipeline === "audio" && (
        <div className="topic-filter" style={{ marginTop: "1rem" }}>
          <div className="range-head">
            ...
            <p>Server-side transcription converts audio to text and then splits it into paragraphs (requires OPENAI_API_KEY).</p>
          </div>
          <label className="field">
      ...
      {sourceMaterialPipeline === "document" && (
        <div className="topic-filter" style={{ marginTop: "1rem" }}>
          <div className="range-head">
            <strong>Upload text / transcript file</strong>
            <p>Supports .txt, .md, .srt, and .vtt. Files with timestamps appear as time blocks.</p>
          </div>
          <label className="field">
            <span>File</span>
            <input
      ...
      {genericSegments.length > 0 && sourceMaterialPipeline !== "transcript" && (
        <div className="section-workspace" style={{ marginTop: "1rem" }}>
          <div className="range-head">
            ...
            <p>Select the blocks to analyze or save. Forum and post imports keep paragraph structure where possible.</p>
          </div>
      ...
      <details open className="priority-section" style={{ marginTop: "1rem" }}>
        ...
        <div className="timestamp-chapters">
          ...
          {selectedMaterial ? (
            ...
            <p className="transcript-note">No source material selected yet. Select sections or paragraphs, or enable “Use full source.”</p>
          )}
      ...
      <details open className="priority-section" style={{ marginTop: "0.5rem" }}>
        <summary>Analyze Source (selection only)</summary>
        <div className="timestamp-chapters">
          <p className="transcript-note">These buttons only use the selected topic text. They do not use the full source by default.</p>
          <div className="range-actions cta-row ee-quick-action-grid">
          ...
          <label className="field">
            ...
            <textarea
              ...
              placeholder="Example: find the best passages for a healing essay; extract the main ideas from this podcast transcript…"
            />
          </label>
          <div className="range-actions cta-row">
            <button
        ...
        <div className="transcript-empty">
          ...
          <p>
            {effectiveYoutubeSource
              ? "Extract the source, then select timestamp chapters, rough sections, or topic matches before writing to Source."
              : "Paste a YouTube link in Source and return to this tab, or use Link import, Paste long text, or Upload audio for other sources."}
          </p>
          {effectiveYoutubeSource && (
